...
The repo also ships `scripts/install.py`, a uv + Python installer that wraps `helm install` for this chart (the implementation lives in the `scripts/ce_installer/` package; `install.py` is just the launcher). It installs the **published** chart by default and this repo's chart only when given `--chart-path ./charts/mlrun-ce`. The bash predecessor `scripts/install.sh` is still in the tree, but only as the oracle the differential tests check the port against — it is not released and is deleted at cutover. The installer's conventions, phase history and bug log live in [`scripts/AGENTS.md`](scripts/AGENTS.md) — read that when working under `scripts/`; this file covers the chart.
